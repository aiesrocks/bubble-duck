...
| Bubbles | CPU load *(default)*, Claude 5h/weekly, GPU, network, disk | Spawn probability = the chosen metric, normalized 0…1 (`BubbleMetric`). CPU uses a rolling 16-sample average; Claude sources fall back to CPU when there's no usable reading |
